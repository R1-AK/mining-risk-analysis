...
This script contains the core logic for the GEE-based flowpath analysis,
refactored into a main function that can be called by a web server. It uses
the original, one-by-one centroid methodology.
"""
...
def run_flowpath_analysis(mining_shapefile_path, gee_credentials, parameters, risk_indicators):
    ...
    OUT_DIR = Path(f'./outputs_flowpath_{run_timestamp}').resolve()

    BANDS = [(0, 1_000, '0_1km'), (1_000, 5_000, '1_5km'),
             (5_000, 10_000, '5_10km'), (10_000, 20_000, '10_20km')]
    ...
    def download_data_local(centroids_gdf, indicators):
        ...
        for f in tqdm(feat_list, desc="Downloading GEE data"):
            ...
            robust_ee_download(dem_img, region, OUT_DIR / 'dem_tiles' / f'dem_{lease_id}.tif', scale=SCALE)
            source_mask = ee.Image().toByte().paint(ee.Feature(f), 1).selfMask()
            robust_ee_download(source_mask, region, OUT_DIR / 'source_masks' / f'source_{lease_id}.tif', scale=SCALE)

            if 'population' in indicators:
                robust_ee_download(pop_img, region, OUT_DIR / 'pop_tiles' / f'pop_{lease_id}.tif', scale=100)
            if 'forest' in indicators or 'agriculture' in indicators:
                robust_ee_download(lc_img, region, OUT_DIR / 'lc_tiles' / f'lc_{lease_id}.tif', scale=100)
    ...
    def process_flowpaths_local(centroids_gdf):
        ...
        for _, lease in tqdm(list(centroids_gdf.iterrows()), desc="Processing flowpaths"):
            ...
            dem_path = OUT_DIR / 'dem_tiles' / f'dem_{lease_id}.tif'
            source_path = OUT_DIR / 'source_masks' / f'source_{lease_id}.tif'
            out_path = OUT_DIR / 'flowpath_tiles' / f'flowpath_{lease_id}.tif'
            ...
            try:
                ...
                stepmap = {1: (0, 1), 2: (1, 1), 4: (1, 0), 8: (1, -1), 16: (0, -1), 32: (-1, -1), 64: (-1, 0),
                           128: (-1, 1)}
    ...
    def run_impact_analysis_local(leases_gdf, indicators):
        ...
        for _, lease in tqdm(list(leases_gdf.iterrows()), desc="Analyzing lease impacts"):
            lease_id = lease.lease_id
            flow_path = OUT_DIR / 'flowpath_tiles' / f'flowpath_{lease_id}.tif'
            ...
            if 'population' in indicators:
                pop_path = OUT_DIR / 'pop_tiles' / f'pop_{lease_id}.tif'
                if pop_path.exists():
                    with rasterio.open(pop_path) as pop_src:
                        pop_arr_reproj = np.zeros(dst_shape, dtype=np.float32)
                        warp.reproject(pop_src.read(1), pop_arr_reproj, src_transform=pop_src.transform,
                                       src_crs=pop_src.crs, dst_transform=dst_transform, dst_crs=dst_crs)
            ...
            if 'forest' in indicators or 'agriculture' in indicators:
                lc_path = OUT_DIR / 'lc_tiles' / f'lc_{lease_id}.tif'
                if lc_path.exists():
                    with rasterio.open(lc_path) as lc_src:
                        lc_arr_reproj = np.zeros(dst_shape, dtype=np.uint8)
                        warp.reproject(lc_src.read(1), lc_arr_reproj, src_transform=lc_src.transform,
                                       src_crs=lc_src.crs, dst_transform=dst_transform, dst_crs=dst_crs,
                                       resampling=warp.Resampling.nearest)
                        area_km2 = (SCALE ** 2) / 1e6
                        if 'forest' in indicators:
                            forest = ((lc_arr_reproj >= 111) & (lc_arr_reproj <= 126))
                            for qi, qf, tag in BANDS:
                                lease_result[f'forest_km2_{tag}'] = float(
                                    np.sum(forest[(dist_img >= qi) & (dist_img < qf)]) * area_km2)
                        if 'agriculture' in indicators:
                            agri = (lc_arr_reproj == 40)
                            for qi, qf, tag in BANDS:
                                lease_result[f'agri_km2_{tag}'] = float(
                                    np.sum(agri[(dist_img >= qi) & (dist_img < qf)]) * area_km2)
    ...
    leases_with_results = leases_orig.merge(results_df, on='lease_id', how='left')

    out_gpkg = OUT_DIR / 'leases_with_flowpath_results.gpkg'
    leases_with_results.to_file(out_gpkg, layer='leases_with_results', driver='GPKG')

    results_path = OUT_DIR / "flowpath_analysis_summary.xlsx"
